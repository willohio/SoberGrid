...
@interface XHMessageTableViewController () <JFDepthViewDelegate,SavedPhraseControllerDelegate,PhotosViewControllerDelegate>
 ...
 *  @param photo 目标图片
 */
- (void)didSendMessageWithPhoto:(UIImage *)photo fromSentList:(BOOL)status;
/**
...
// Agile code
- (void)didSendSavedPhraseWithString:(NSString*)phrase;
...
- (XHShareMenuView *)shareMenuView {
    if (!_shareMenuView) {
        XHShareMenuView *shareMenuView = [[XHShareMenuView alloc] initWithFrame:CGRectMake(0, CGRectGetHeight(self.view.bounds), CGRectGetWidth(self.view.bounds), 64)];
...
- (void)setup {
    ...
    _allowsSendVoice = false;
    _allowsSendMultiMedia = YES;
    _allowsSendFace = false;
    _allowsSendButton = true;
...
- (void)initilzer {
    ...
    messageTableView.backgroundColor = SG_BACKGROUD_COLOR;
    ...
    inputView.allowsSendButton = self.allowsSendButton;
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    [self.messageInputView.inputTextView addObserver:self
                                     forKeyPath:@"contentSize"
                                        ...
                                        context:nil];
    if (_photoesController || self.topViewController) {
...
- (void)didSendMessageWithPhoto:(UIImage *)photo fromSentList:(BOOL)status {
    ...
    if ([self.delegate respondsToSelector:@selector(didSendPhoto:fromSender:onDate:fromSentList:)]) {
        [self.delegate didSendPhoto:photo fromSender:self.messageSender onDate:[NSDate date] fromSentList:status];
    }
}
...
- (void)didSendSavedPhraseWithString:(NSString *)phrase{
    DLog(@"send SavedPhrase : %@", phrase);
    if ([self.delegate respondsToSelector:@selector(didSendSavedPhrase:fromSender:onDate:)]) {
        [self.delegate didSendSavedPhrase:phrase fromSender:self.messageSender onDate:[NSDate date]];
    }
}
#pragma mark - Other Menu View Frame Helper Mehtod
...
- (BOOL)inputTextViewShouldBeginEditing:(XHMessageTextView *)messageInputTexView{
    if(_photoesController || self.topViewController){
        return false;
    }
    return true;
...
- (void)didSelectedMultipleMediaActionWithState:(BOOL)status {
    DLog(@"didSelectedMultipleMediaAction");
    self.textViewInputViewType = XHInputViewTypeShareMenu;
    [self layoutOtherMenuViewHiden:status];
}
...
- (void)didSelecteShareMenuItem:(XHShareMenuItem *)shareMenuItem atIndex:(NSInteger)index {
    ...
    void (^PickerMediaBlock)(UIImage *image, NSDictionary *editingInfo) = ^(UIImage *image, NSDictionary *editingInfo) {
        if (image) {
            [weakSelf didSendMessageWithPhoto:image fromSentList:false];
        } else {
            if (!editingInfo)
                return ;
            ...
            if (CFStringCompare ((__bridge CFStringRef) mediaType, kUTTypeMovie, 0) == kCFCompareEqualTo) {
                ...
                UIImage *image = [editingInfo valueForKey:UIImagePickerControllerEditedImage];
                [weakSelf didSendMessageWithPhoto:[image rotate] fromSentList:false];
            }
        }
    };
    switch (index) {
        case 0: {
            // Agile code to select source type
            MEAlertView *alerView = [[MEAlertView alloc]initWithTitle:NSLocalizedString(@"Select source type", nil) message:nil delegate:self cancelButtonTitle:NSLocalizedString(@"Cancel", nil) otherButtonTitles:NSLocalizedString(@"Camera", nil),NSLocalizedString(@"Photo Gallery", nil), nil];
            alerView.Object = PickerMediaBlock;
            [alerView show];
        ...
        case 1: {
            ...
            if(_photoesController || self.topViewController || [_messageInputView.inputTextView isFirstResponder]){
                return;
            }
            _photoesController = [[PhotosViewController alloc] init];
            _photoesController.isMyPhotoes = true;
            _photoesController.delegate = self;
            UITapGestureRecognizer* tapRec = [[UITapGestureRecognizer alloc] initWithTarget:self.photoesController action:@selector(closeView:)];
            ...
            self.depthView.blurAmount  = JFDepthViewBlurAmountLight;
            self.photoesController.depthViewReference = self.depthView;
            self.photoesController.presentedInView = self.view;
            [self.depthView presentViewController:self.photoesController inView:self.view animated:YES];
            break;
        }
        case 2:{
            if ([[SoberGridIAPHelper sharedInstance] getTypeOfSubsciption] == kSGSubscriptionTypeNone) {
            ...
            _photoesController = [[PhotosViewController alloc] init];
            _photoesController.isMyPhotoes = false;
        ...
        case 3: {
            WEAKSELF
            [self.locationHelper getCurrentGeolocationsCompled:^(NSArray *placemarks,CLLocation *location) {
                if (location) {
                    [weakSelf didSendGeolocationsMessageWithGeolocaltions:nil location:location];
                }else{
                    UIAlertView *alertView=[[UIAlertView alloc]initWithTitle:nil message:@"Please enable Location services from settings" delegate:nil cancelButtonTitle:@"OK" otherButtonTitles:nil];
                    [alertView show];
                }
            }];
            break;
        ...
        case 4 :{
            ...
            if(self.topViewController || self.photoesController || [_messageInputView.inputTextView isFirstResponder])
                return;
            self.topViewController = [[SavedPhraseController alloc] init];
            self.topViewController.delegate = self;
            UITapGestureRecognizer* tapRec = [[UITapGestureRecognizer alloc] initWithTarget:self.topViewController action:@selector(closeView:)];
            self.depthView = [[JFDepthView alloc] init];
            self.depthView.delegate = self;
...
#pragma mark - JFDepthView delegates
...
- (void)didDismissDepthView:(JFDepthView*)depthView {
    ...
    if(_photoesController)
    _photoesController= nil;
...
- (void)didSelectedPhrase:(NSString *)strPhrase{
    [self didSendSavedPhraseWithString:strPhrase];
}
- (void)photosViewControllerdidFinishPickingPhoto:(UIImage *)photo{
    [self didSendMessageWithPhoto:photo fromSentList:true];
}
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex{
    ...
    if (buttonIndex == 1) {
        ...
        [self.photographyHelper showOnPickerViewControllerSourceType:UIImagePickerControllerSourceTypeCamera onViewController:self compled:meAlert.Object];
    }
    if (buttonIndex == 2) {
         [self.photographyHelper showOnPickerViewControllerSourceType:UIImagePickerControllerSourceTypePhotoLibrary onViewController:self compled:meAlert.Object];
    }
}
