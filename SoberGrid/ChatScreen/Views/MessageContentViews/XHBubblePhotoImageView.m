...
- (void)configureMessagePhoto:(UIImage *)messagePhoto thumbnailUrl:(NSString *)thumbnailUrl originPhotoUrl:(NSString *)originPhotoUrl onBubbleMessageType:(XHBubbleMessageType)bubbleMessageType {
    ...
    if (thumbnailUrl) {
        ...
        [manager downloadImageWithURL:[NSURL URLWithString:thumbnailUrl] options:SDWebImageRetryFailed progress:nil completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, BOOL finished, NSURL *imageURL) {
            if (weakSelf.bubbleMessageType == XHBubbleMessageTypeReceiving) {
                [image saveToTempDirectoryofType:false withName:[imageURL lastPathComponent]];
            }
...
//            // Agile code to save receiving photoes
//           
//                    }];
    }
}
