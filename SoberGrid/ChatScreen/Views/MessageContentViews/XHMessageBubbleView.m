...
+ (CGFloat)neededWidthForText:(NSString *)text {
   // CGSize stringSize;
    CGRect  stringRect = [text boundingRectWithSize:CGSizeMake(MAXFLOAT, 19) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName: [[XHMessageBubbleView appearance] font]} context:nil];
    
   // stringSize = [text sizeWithFont:[[XHMessageBubbleView appearance] font]
                     //constrainedToSize:CGSizeMake(MAXFLOAT, 19)];
    return roundf(stringRect.size.width);
}
...
- (instancetype)initWithFrame:(CGRect)frame
    ...
    if (self) {
        ...
        if (!_displayTextView) {
            ...
            displayTextView.font = SGREGULARFONT(16.0);
