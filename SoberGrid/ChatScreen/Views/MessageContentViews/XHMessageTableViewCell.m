...
- (void)configAvatorWithMessage:(id <XHMessageModel>)message {
    if (message.avator) {
        [self.avatorButton setImage:[XHMessageAvatorFactory avatarImageNamed:message.avator messageAvatorType:XHMessageAvatorTypeCircle] forState:UIControlStateNormal];
        if (message.avatorUrl) {
            self.avatorButton.messageAvatorType = XHMessageAvatorTypeCircle;
            [self.avatorButton setImageWithURL:[NSURL URLWithString:message.avatorUrl] placeholer:[UIImage imageNamed:@"avator"]];
        }
    } else {
        [self.avatorButton setImage:[XHMessageAvatorFactory avatarImageNamed:[UIImage imageNamed:@"avator"] messageAvatorType:XHMessageAvatorTypeCircle] forState:UIControlStateNormal];
    }
}
...
- (instancetype)initWithMessage:(id <XHMessageModel>)message
    ...
    if (self) {
        ...
        if (!_timestampLabel) {
            ...
            timestampLabel.font = SGREGULARFONT(13.0);
        ...
        avatorButton.layer.cornerRadius = avatorButton.frame.size.height / 2;
...
//        self.userNameLabel = userNameLabel;
        ...
        // 4、配置需要显示什么消息内容，比如语音、文字、视频、图片
        // 4.Configuration needs to show what the message content , such as voice , text , video, pictures
