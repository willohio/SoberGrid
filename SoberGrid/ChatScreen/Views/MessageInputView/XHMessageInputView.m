...
#define kXHTouchToRecord         @"Press and hold the talk"
#define kXHTouchToFinish         @"Loosen the end"
...
@property (nonatomic, weak, readwrite) UIButton *messageSendButton;
...
- (void)messageStyleButtonClicked:(UIButton *)sender {
    ...
    switch (index) {
        ...
        case 2: {
            ...
            if ([self.delegate respondsToSelector:@selector(didSelectedMultipleMediaActionWithState:)]) {
                [self.delegate didSelectedMultipleMediaActionWithState:sender.selected];
            }
            sender.selected = !sender.selected;
            break;
        }
...
//        self.voiceChangeButton = button;
    ...
    // Allowed to send multimedia messages , why not first put face buttons do ? Because of the need layout！
    if (self.allowsSendMultiMedia) {
        button = [[UIButton alloc]initWithFrame:CGRectMake(0, 0, 36, 36)];
        [button setTitle:@"+" forState:UIControlStateNormal];
        [button setTitle:@"x" forState:UIControlStateSelected];
        [button setTitleColor:[UIColor darkGrayColor] forState:UIControlStateNormal];
        button.titleLabel.font = [UIFont boldSystemFontOfSize:36.0];
    ...
    if (self.allowsSendButton) {
        button = [[UIButton alloc]initWithFrame:CGRectMake(0, 0, 50, [XHMessageInputView textViewLineHeight])];
       ...
       // [button setImage:[UIImage imageNamed:@"keyborad"] forState:UIControlStateSelected];
        ...
        button.tag = 3;
        buttonFrame = button.frame;
        buttonFrame.origin = CGPointMake(CGRectGetWidth(self.bounds) - (allButtonWidth ? allButtonWidth : (textViewLeftMargin * 2)) + textViewLeftMargin - CGRectGetWidth(buttonFrame) - horizontalPadding, verticalPadding);
        allButtonWidth += CGRectGetWidth(buttonFrame) + horizontalPadding * 1.5;
//        if (self.allowsSendMultiMedia) {
//            buttonFrame.origin = CGPointMake(CGRectGetMinX(self.multiMediaSendButton.frame) - CGRectGetWidth(buttonFrame) - horizontalPadding, verticalPadding);
//            allButtonWidth += CGRectGetWidth(buttonFrame) + horizontalPadding * 1.5;
...
//        }
    ...
    textView.placeHolder = NSLocalizedString(@"Message", nil);
...
- (BOOL)textViewShouldBeginEditing:(UITextView *)textView {
    ...
    if ([self.delegate respondsToSelector:@selector(inputTextViewShouldBeginEditing:)]) {
        return [self.delegate inputTextViewShouldBeginEditing:self.inputTextView];
    }
    self.faceSendButton.selected = NO;
