...
@synthesize normaltextColor,mentionTextColor,hashtagTextColor,linkTextColor,selectedMentionTextColor,selectedHashtagTextColor,selectedLinkTextColor;
@synthesize delegate;
...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        mentionTextColor = [UIColor blueColor];
        hashtagTextColor = [UIColor blueColor];
...
- (void)setString:(NSString *)string
{
    ...
    [attrString addAttribute:NSForegroundColorAttributeName value:normaltextColor range:[string rangeOfString:string]];
...
- (CGRect)rectForLetterAtIndex:(NSUInteger)index
{
    ...
    for ( ; i < len; i++ )
    {
        ...
        CGRect currentSizeRect=[currentLine boundingRectWithSize:CGSizeMake(self.frame.size.width, 1000) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:self.font} context:nil];
       // CGSize currentSize = [currentLine sizeWithFont:self.font constrainedToSize:CGSizeMake(self.frame.size.width, 1000) lineBreakMode:self.lineBreakMode];
        currentWidth = currentSizeRect.size.width;
        
        if ( currentSizeRect.size.height > self.font.lineHeight && currentWidth != 0)
        {
    ...
    CGRect totalSizeRect = [self.text boundingRectWithSize:CGSizeMake(self.frame.size.width, 100000) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:self.font} context:nil];
    ...
    if ( totalSizeRect.size.height > self.frame.size.height )
    {
    ...
    currentWidth = [[self.text substringWithRange:NSMakeRange(lineStartsOn, currentLineLength)] sizeWithAttributes:@{NSFontAttributeName:self.font}].width;
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    if(currentSelectedString != nil)
    {
        ...
        {
            [newAttrString addAttribute:NSForegroundColorAttributeName value:mentionTextColor range:currentSelectedRange];
            if ([delegate respondsToSelector:@selector(selectedMention:)]) {
        ...
        {
            [newAttrString addAttribute:NSForegroundColorAttributeName value:hashtagTextColor range:currentSelectedRange];
            if ([delegate respondsToSelector:@selector(selectedHashtag:)]) {
        ...
        {
            [newAttrString addAttribute:NSForegroundColorAttributeName value:linkTextColor range:currentSelectedRange];
            [self linkSelected:currentSelectedString];
...
- (void)linkSelected:(NSString *)strLink{
    [[NSNotificationCenter defaultCenter]postNotificationName:NOTIFICATION_LINKTAPPED object:strLink];
    if ([delegate respondsToSelector:@selector(selectedLink:)]) {
        [delegate selectedLink:strLink];
        
    }
}
- (void)hashTagSelected:(NSString *)strHashTag{
