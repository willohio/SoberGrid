...
- (void)loadVideosFromAlbum:(NSString *)albumName
                 completion:(void (^)(NSMutableArray *, NSError *))completion
{
    __block BOOL gropFound = false;
    ALAssetsLibraryGroupsEnumerationResultsBlock block = ^(ALAssetsGroup *group, BOOL *stop) {
        // Checking if library exists
        if (group == nil){
            if (!gropFound) {
        ...
        if ([albumName compare:[group valueForProperty:ALAssetsGroupPropertyName]] == NSOrderedSame) {
            ...
            [group enumerateAssetsUsingBlock:^(ALAsset *result, NSUInteger index, BOOL *stop) {
              ...
              //  ALAssetRepresentation *imageRep = [result defaultRepresentation];
                ...
                [d setObject:result forKey:@"Movie"];
                if (image) {
                     [d setObject:image forKey:@"Image"];
                }
            ...
            // Execute the |completion| block
            if (completion) completion(assets, nil);
...
- (void)deleteVideo:(ALAsset*)asset FromAlbum:(NSString *)albumName
                 completion:(void (^)(NSMutableArray *, NSError *))completion
{
    ALAssetsLibraryGroupsEnumerationResultsBlock block = ^(ALAssetsGroup *group, BOOL *stop) {
        // Checking if library exists
        ...
        if ([albumName compare:[group valueForProperty:ALAssetsGroupPropertyName]] == NSOrderedSame) {
            ...
            [group enumerateAssetsUsingBlock:^(ALAsset *result, NSUInteger index, BOOL *stop) {
                ...
                // Getting the image from the asset
                if ([[[result defaultRepresentation].url absoluteString] isEqualToString:[[asset defaultRepresentation].url absoluteString]]) {
